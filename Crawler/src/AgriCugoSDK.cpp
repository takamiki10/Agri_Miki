...
// Create a Ticker object for the timer
Ticker timer0;
...
void IRAM_ATTR cugo_timer_handler0()
{
    ld2_get_cmd(); // Handle the timer interrupt
}
...
void cugo_init()
{
    ...
    delay(1000);                      // LD-2起動待機
    Serial1.begin(115200, SERIAL_8N1, RX_PIN, TX_PIN); // BLDCとの通信

    ld2_set_feedback(2, 0b10000001); // Set feedback frequency and data types
    delay(1000);

    // Set the timer with Ticker (replacing ESP32Timer)
    timer0.attach_ms(cugo_ld2_feedback_dutation, cugo_timer_handler0);
...
void ld2_set_encoder(unsigned char frame[12]) {
  ...
  if (cugo_prev_encoder_L > 0 && encoderL < 0 && (cugo_prev_encoder_L - encoderL) > (CUGO_LD2_COUNT_MAX / 2)) {
    cugo_current_count_L += CUGO_LD2_COUNT_MAX - cugo_prev_encoder_L + encoderL;
  } else if (cugo_prev_encoder_L < 0 && encoderL > 0 && (encoderL - cugo_prev_encoder_L) > (CUGO_LD2_COUNT_MAX / 2)) {
    cugo_current_count_L += CUGO_LD2_COUNT_MAX + cugo_prev_encoder_L - encoderL;
  } else {
    cugo_current_count_L += encoderL - cugo_prev_encoder_L;
  }
  cugo_prev_encoder_L = encoderL;

  if (cugo_prev_encoder_R > 0 && encoderR < 0 && (cugo_prev_encoder_R - encoderR) > (CUGO_LD2_COUNT_MAX / 2)) {
    cugo_current_count_R += CUGO_LD2_COUNT_MAX - cugo_prev_encoder_R + encoderR;
  } else if (cugo_prev_encoder_R < 0 && encoderR > 0 && (encoderR - cugo_prev_encoder_R) > (CUGO_LD2_COUNT_MAX / 2)) {
    cugo_current_count_R += CUGO_LD2_COUNT_MAX + cugo_prev_encoder_R - encoderR;
  } else {
    cugo_current_count_R += encoderR - cugo_prev_encoder_R;
  }
...
void cugo_reset() {
    esp_restart();  // Reboots the ESP32 system
    while (true);   // Infinite loop after restarting (just for safety)
}
